...
<!-- visual:start -->
{% include archify-diagram.html src="/assets/diagrams/11_observability.html" title="ClientからOriginまでの観測ポイント" summary="CF-Rayなどの相関情報を使い、Client・Edge・Worker・Originのどのレイヤーで遅延やエラーが起きたかを追える状態を作る。" %}
<!-- visual:end -->
...
<!-- visual:start -->
{% include archify-diagram.html src="/assets/diagrams/11_debug_runbook.html" title="障害調査Runbook" summary="Client側の事実から始め、Headers → Edge → Worker → Originの順で切り分けると無駄な設定変更を減らせる。" %}
<!-- visual:end -->
